...
function speak(text){
  try{ speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.rate=.85; speechSynthesis.speak(u); }catch(e){}
}
function openGame(name){
  const game = games[name] || {icon:'⭐', prompt:`${name} is coming soon!`, answers:['Earn Star'], correct:'Earn Star'};
  modalIcon.textContent = game.icon; modalTitle.textContent = name; modalPrompt.textContent = game.prompt; result.textContent=''; choices.innerHTML='';
  game.answers.forEach(answer=>{
    const btn=document.createElement('button'); btn.type='button'; btn.textContent=answer;
    btn.addEventListener('click',()=>{
      if(answer===game.correct){ result.textContent='Great job, princess! +1 star ⭐'; earnStars(1); speak('Great job! You earned a star.'); }
      else { result.textContent='Good try. Try again!'; speak('Good try. Try again.'); }
    });
    choices.appendChild(btn);
  });
  modal.showModal();
  speak(`${name}. ${game.prompt}`);
}
...
document.querySelectorAll('[data-nav]').forEach(el=>el.addEventListener('click',()=>{
  ...
  if(name==='Achievements'){ openGame('Achievements'); return; }
  if(name==='menu'){ alert(`Harper's STEM Learning v${VERSION}
Stars are saved on this device.`); return; }
  alert(`${name} area is ready for more princess adventures!`);
}));
